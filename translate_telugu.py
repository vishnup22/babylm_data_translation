...
def clean_sentences(sentences):
    """Filter out empty, too-short, or non-ASCII-decodable lines."""
    cleaned = []
    for s in sentences:
        s = s.strip()
        if len(s) < 2:
...
def translate_batch(sentences, model, tokenizer, ip, device):
    # filter valid sentences, keep track of indices
    ...
    batch = ip.preprocess_batch(valid_sentences, src_lang=SRC_LANG, tgt_lang=TGT_LANG)
    ...
    for idx, translation in zip(valid_idx, translated):
...
def translate_file(input_path, output_path, model, tokenizer, ip, accelerator, output_dir, split):
    ...
    ckpt_file = Path(output_dir) / f".ckpt_te_{split}_{filename}"
    ...
    for chunk_idx in range(num_chunks):
        ...
        local_lines = clean_sentences(local_lines)
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./babylm_telugu")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"English -> Telugu | split={args.split} (source: {cfg['hf_repo']}) | "
                    f"{accelerator.num_processes} GPUs | output: {args.output_dir}")

    tokenizer = AutoTokenizer.from_pretrained(args.model_id, trust_remote_code=True)

    # load model directly to each GPU — do NOT use accelerator.prepare()
    # IndicTrans2 is a seq2seq inference model; DDP wrapping breaks .generate()
    ...
    if accelerator.is_main_process:
        logger.info(f"Telugu {args.split} translation complete")
